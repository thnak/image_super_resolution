...
class ResNet(nn.Module):
    def __init__(self, num_block_resnet=16, add_rate=0.2, scaleRate = 2):
        ...
        residual = [RRDB(64, 3,
                         act=nn.LeakyReLU(), add_rate=add_rate) for _ in range(num_block_resnet)]
        # residual = [ResidualBlock1(64, 64, 64, 3, nn.LeakyReLU()) for _ in range(num_block_resnet)]
        self.residual = nn.Sequential(*residual)

        self.conv1 = Conv(64, 64, 3, 1, None, act=False)
        scaler = [Scaler(64, 64,
                         2, 3,
                         nn.LeakyReLU()) for _ in range(scaleRate)]
...
class EResNet(nn.Module):
    def __init__(self, num_block_resnet=16, add_rate=0.2, scaleRate=2):
        super().__init__()
        scaleRate = scaleRate // 2
        self.conv0 = ConvWithoutBN(3, 64, 9, 1, None, act=nn.LeakyReLU())
        residual = [RRDB(64, 3,
                         act=nn.LeakyReLU(), add_rate=add_rate, use_BN=False) for _ in range(num_block_resnet)]
        # residual = [ResidualBlock1(64, 64, 64, 3, nn.LeakyReLU()) for _ in range(num_block_resnet)]
        self.residual = nn.Sequential(*residual)
